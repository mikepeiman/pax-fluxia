...
4. **Commit by explicit pathspec — the git index is SHARED.** Staging is not per-agent: `git add` then a bare `git commit` can sweep your staged files into another agent's commit (or theirs into yours — observed live on 2026-06-21). Commit atomically with `git commit -- <your exact paths>` (stage any *new* files first, then `git commit -- <paths>`). **Never** `git add -A`, `git add <dir>`, or `git commit -a`. Small, frequent commits keep the collision window tiny.
