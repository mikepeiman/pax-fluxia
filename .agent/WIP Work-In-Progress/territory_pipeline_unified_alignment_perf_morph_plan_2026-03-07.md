...
- Added close-zoom fidelity guard in vector extraction: automatic reduction of straightening and simplify tolerance as zoom increases to reduce local border/fill drift while preserving low-zoom readability.
- Architecture correction: removed CPU vector overlay path from the live border pipeline (debug-only gate).
- Canonical runtime now always routes border rendering through the GPU ownership-field two-pass path when renderer is available.
- Rationale: vector simplify/straighten can never guarantee exact ownership parity; ownership texture + boundary distance is the only deterministic border/fill alignment source currently in code.
