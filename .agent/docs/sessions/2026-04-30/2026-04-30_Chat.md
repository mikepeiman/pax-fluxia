...
34. Dangit, the landing page is still loop reloading. FIX THIS PLEASE

35. Add some quick diagnostic logs to ensure success

36. You do not do this. You will *never* instruct me to use the console to change game settings. Have you noticed the settings surface I have exposed in UI? Have you noticed any agent rules around settings and tuning? C'mon man.
`If you intentionally want live settings dumps again, use ?settingsDump=1 in the URL or set localStorage["pax-enable-settings-dump"] = "true". Then you should see settings_dump_enabled and the post events.`

Add these to Debug panel.

37. Assistant work summary
- Kept the landing-loop diagnostic logs in `pax-fluxia/src/lib/utils/settingsDump.ts`, but moved the activation path into UI instead of console guidance.
- Added `pax-fluxia/src/lib/components/ui/settings/SettingsDumpDiagnosticsControls.svelte` and mounted it from `pax-fluxia/src/lib/components/ui/settings/ControlsSection-Diagnostics.svelte`.
- The new Diagnostics subsection is `Live Settings Dump`, with a `Continuous Settings Dump` toggle, `Dump Now` action, status readouts, and target-path visibility.
- `settingsDump.ts` now exports `settingsDumpState`, `setSettingsDumpEnabled()`, `dumpSettingsNow()`, and `refreshSettingsDumpState()` so Diagnostics owns the feature.
- The helper now reports UI-owned enablement and logs `settings_dump_ui_toggled` / `settings_dump_manual_requested` instead of relying on user-facing URL/query instructions.
