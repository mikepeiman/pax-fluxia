...
- “Removed from UI” was not sufficient for the rules/runtime cleanup; theme and settings-plumbing references also needed to be cut so the options were truly expunged.
...
- The correct theme cleanup was not “delete extra dropdowns,” but “promote one full-capability theme manager to the game menu and remove the Settings-panel copy entirely.”
...
- When a “current settings” baseline has effectively become the product default, it should be promoted once into both config startup defaults and the named built-in theme instead of being left as an implicit local state snapshot.
