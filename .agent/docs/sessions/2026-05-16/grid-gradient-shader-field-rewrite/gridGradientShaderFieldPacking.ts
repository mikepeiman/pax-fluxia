import type { GridRenderCell, GridVRole } from '../../metaballGrid/metaballGridTypes';
...
function buildPalette(params: BuildGridGradientShaderFieldTexturePlanParams): {
    ...
    for (let colorIdx = 0; colorIdx < fillColors.length; colorIdx += 1) {
        ...
        packHexColor(fillColors[colorIdx] ?? 0xffffff, params.settings.fillAlpha, paletteTextureData, paletteIndex * 4);
...
export function buildGridGradientShaderFieldTexturePlan(
    ...
    return {
        ...
        distanceBuildMs: 0,
        ownerSummaryBuildMs: ownerMapBuildMs + paletteBuildMs,
