...
- Viewport-edge background continuity is a shell-ownership problem plus a world-centering problem; fixing only one of those leaves visible seams.
- A shell extension layer cannot be hardcoded to one decorative asset if gameplay supports live background image swaps; it must follow the same runtime background selection path as the canvas background.
...
- The broader â€œeverything becomes conquestâ€ symptom had a second root cause upstream: PREV-frame capture based on idle-only caching is not safe once conquests chain back-to-back.
