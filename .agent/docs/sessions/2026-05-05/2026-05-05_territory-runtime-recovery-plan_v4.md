# 2026-05-05 Territory Runtime Recovery Plan v4
...
This v4 plan keeps the v3 recovery sequence, but tightens four things that were still too loose:

1. a diagnostics mode is added that freezes on unclassified boundaries instead of letting them degrade silently into snap or bad transport
2. `CX`, `LP`, `DX`, and `MSR` are defined as explicit shared geometry constraints, not only as legacy tuning names
3. `starWeight` is separated cleanly from `MSR`
4. the current virtual-site heuristics are reframed as temporary compiler adapters, not the final truth model
...
- keep as a separate scalar on the star-site input to the geometry solve
- expose as a distinct tuning surface
- remove the misleading `starMargin` name from user-facing and semantic code paths
...
- Minimum Star Range
- a star-protection constraint
- boundaries and constraint samples that do not originate from a star must not intrude inside that star's protected disk
...
2. during corridor / lane-pair / disconnect sample generation:
   - do not place non-endpoint samples inside another star's `MSR` disk
3. during the geometry solve:
   - materialize each protection disk as a guard ring of same-owner supplemental samples around the star
   - this makes the protection disk explicit in the solve instead of faking it through site weight
...
- `MSR` is not `starWeight`
- `MSR` is a protection constraint
...
If a conquest-local boundary cannot be classified, the runtime should freeze and export truth instead of continuing into an unexplained snap or deformation.
...
If a case cannot be bounded locally and cleanly, snap remains preferable to gross deformation.
...
- a classified explicit snap does not trigger this freeze
