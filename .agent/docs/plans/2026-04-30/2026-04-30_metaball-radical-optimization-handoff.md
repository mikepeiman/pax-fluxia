...
- If the active mode really reaches roughly 32k influence sources, a naive or semi-naive solve is expensive even before drawing:
  - CPU cost: `samples × covered cells` in `accumulateSamplesIntoFields()`
...
- Why this matters for merge:
  ...
  - that failure mode matches the user’s report very closely:
...
- Trigger:
  - after the topology-loop coverage fix, user reported two remaining coupled issues:
    - borders were still appearing inside a player’s own territory
...
- Intended effect:
  ...
  - suppress the internal “borders inside my own region” artifact that was being drawn around those voids
...
- Deterministic diagnosis:
  ...
  - the breakage was not “missing geometry”
