...
### 2026-05-04 additive note

- Fixed a clean-runtime crash caused by stale legacy territory ids crossing into the unified worker contract.
- User-visible failure:
  - `Unknown geometry mode: canonical_power_voronoi`
- Root cause:
  - worker/runtime geometry registry now only supports `unified_vector`
  - but two old seams still emitted retired ids:
    - `GameCanvas.svelte` special-case for `power_voronoi_canonical`
    - `ControlsSection-Territory.svelte` legacy geometry/fill transition options
  - runtime normalization was not defensive enough against direct stale callers
- Implemented in:
...
- Added regression coverage:
  ...
  - expanded `pax-fluxia/src/lib/territory/integration/TerritorySettingsBridge.test.ts`
- Merge/backport guidance:
  - if another branch/worktree still crashes on `canonical_power_voronoi`, do not patch only the registry
  - the correct fix is contract repair at all three boundaries:
    1. runtime normalizer
    2. direct bridge selection builders
...
This improved things, but did not fully satisfy the userâ€™s deeper invariant.
...
- the original report may not have been sourced from this worktreeâ€™s visible behavior
...
The userâ€™s objections included:
...
The branch improves the architecture and fixes multiple inert-control and UI-ownership failures, but the user has already identified that fill/border and steady-state/transition parity are still not fully solved. Treat this branch as an active integration branch, not as â€œready for blind fast-forward to master.â€
...
- UI follow-up:
  - the border section now tells the user that `Inward Offset` lives in `Fill`, which avoids another false â€œthe control was removedâ€ read without duplicating the control
- Validation after this additive update:
  - `bun x vitest run src/lib/territory/frontier/frontier.test.ts src/lib/territory/families/metaballGrid/MetaballGridFamily.test.ts tools/debug/benchmark-frontier-techniques.test.ts`
  - `bun x vite build`
...
- This invalidated an important part of the earlier 2026-05-01 correction:
  ...
  - the userâ€™s requirement is that the authored/display map rect, not the star cloud, owns the visible filled map area
