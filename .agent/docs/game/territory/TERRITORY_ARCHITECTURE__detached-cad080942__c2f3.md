# Territory Architecture â€” Reference Specification

**Terminology:** In this document, **reference** means "primary agreed design for agents and implementers." It does **not** refer to the Svelte/render mode name `territory_canonical` or any identifier containing "canonical" in code â€” always disambiguate in prose.
...
**Status:** ACTIVE â€” single territory-system design reference.  
**Ref:** D-83
...
|-------|-------------|----------|----------------------|
| **Ownership** | Who owns what. Graph-native, from conquest events. | `OwnershipSnapshot` | `OwnershipLayerCoordinator` â†’ `StarOwnershipSnapshotMode` |
| **Geometry** | Shapes from ownership. Power Voronoi cells, merged territories, shared borders, frontier topology. | `CanonicalGeometrySnapshot` (incl. `FrontierTopology`) | `GeometryLayerCoordinator` â†’ `UnifiedVectorGeometryMode` â†’ `compileVectorGeometry()` |
| **Transition** | **Single coordinated step:** advances one conquest animation. Both region fills and frontier polylines follow the **same** time progression. | `TransitionSnapshot` (containing `FillTransitionFrame` + `BorderTransitionFrame`) | `TransitionLayerCoordinator` â†’ `FrontierTopologyPlanner` + `TopologyFrameSampler` |
| **Presentation** | Drawing to screen. Assembles draw commands; PIXI.Graphics fills, strokes, visual style. | `TerritoryPresentationFrame` (fill + border draw commands) | `PresentationLayerCoordinator` â†’ style modes â†’ `PixiTerritoryPresenter` |
...
**Future:** the architecture keeps **separable hooks** (e.g. optional fill-only or border-only passes) so additional VFX variants can be added without rewriting ownership/geometry. That is an extension point â€” not the default behavior.
...
```
TerritoryFrameInput
    â†’ normalizeTerritoryFrameInput()
    â†’ OwnershipLayerCoordinator.compute()         â†’ OwnershipSnapshot
    â†’ TerritoryWorker.computeGeometrySync()        â†’ CanonicalGeometrySnapshot
    â†’ TransitionLayerCoordinator.compute()         â†’ TransitionSnapshot
    â†’ PresentationLayerCoordinator.compute()       â†’ TerritoryPresentationFrame
    â†’ PixiTerritoryPresenter.present()             â†’ screen
```
...
- **Fill + border on the SAME points.** Every draw call that renders a territory should draw fill AND stroke from the SAME point array â€” `poly(pts).fill()` then `poly(pts).stroke()`. Separate data sources for fills vs borders = guaranteed divergence.
...
- `renderPowerVoronoi()` accepts `state?: PVV2RendererState` â€” always pass state, never rely on module globals
...
| `worldBorderPolylines` | Ownerâ†”world edge polylines |
| `sharedFrontierMap` | D-90 multimap â€” frontiers keyed by `ownerPairKey` (one pair can have multiple disconnected segments) |
| `frontierTopology` | **FrontierTopology** â€” the structural graph consumed by the transition planner |
...
| `FrontierSection` | Shared topological edge between two vertices. Exists EXACTLY ONCE â€” two owners do not get separate copies. Contains full smoothed point array + owner attribution. |
...
**Key invariant:** deterministic equal-density sampling. If two ownership states produce an unchanged frontier span, the point arrays must match exactly â€” stability is recognized by comparison, not heuristics.

---

## 5. Transition Architecture

### Unified Topology Path (target â€” currently gated by `TOPOLOGY_PATH_ENABLED`)

The design-target transition path that produces fills and borders from the **same** interpolated frontier sections:

```
ConquestEvent fires
    â†’ TransitionLayerCoordinator builds envelope (SharedTransitionClock)
    â†’ FrontierTopologyPlanner.buildFrontierTransitionPlan(prevTopo, nextTopo)
        â†’ matchVertices() â€” exact ID + spatial proximity
        â†’ matchSections() â€” by ownerPairKey + endpoint match + midpoint proximity
        â†’ matchLoops() â€” by ownerId + section overlap
        â†’ classify: static | drifted | born | dying
    â†’ each frame: TopologyFrameSampler.sampleTopologyFrame(plan, nextTopo, progress)
        â†’ interpolateSection() per section (OT for drifted, expand/collapse for born/dying)
        â†’ rebuildFillFromSections() â€” walks loop sectionRefs, concatenates interpolated points
        â†’ returns { fillFrame, borderFrame } from the SAME section points
    â†’ PresentationLayerCoordinator consumes unified snapshot
```
...
| Kind | Behavior |
|------|----------|
| `static` | Same points â€” pass through unchanged (zero jitter) |
| `drifted` | Same identity, different points â€” OT interpolation between prev and next |
| `born` | New section â€” expand from midpoint toward full geometry |
| `dying` | Removed section â€” collapse from full geometry toward midpoint |
...
## 6. VFX Integration â€” Concrete Specification

### Existing FX System (verified from code)

```
FXOrchestrator
â”œâ”€â”€ FXClock           â€” pausable game time, speed multiplier
â”œâ”€â”€ VisualStateManager â€” safe mutation API for visual state
â””â”€â”€ FXRegistry         â€” priority-sorted handler dispatch
    â”œâ”€â”€ transfer handlers  â†’ FXHandler<TransferEvent>
    â”œâ”€â”€ combat handlers    â†’ FXHandler<CombatEvent>
    â””â”€â”€ conquest handlers  â†’ FXHandler<ConquestEvent>
```
...
- `FrontierMorphFillMode.ts` â€” the legacy OT polygon-morph fill mode. Known to produce corrupt intermediate frames. Superseded by the unified topology path and `ActiveFrontFillMode`.

### Stale Code Indicators

If you encounter ANY of these patterns, the code is likely obsolete â€” STOP and verify against this document:
...
| `territory/orchestrator/registry.ts` | Method descriptors (contains legacy static/dynamic split â€” pending cleanup) |
| `territory/orchestrator/types.ts` | Type definitions (contains legacy types â€” pending cleanup) |
...
| `territory/layers/geometry/modes/UnifiedVectorGeometryMode.ts` | Sole registered mode â€” delegates to compiler |
...
| `territory/compiler/Geometry_0319.ts` | Power Voronoi generator â€” the computational core |
| `territory/compiler/buildFrontierTopology.ts` | Converts `TerritoryFrontierMap` â†’ `FrontierTopology` |
...
### Layer 3 â€” Transition

| File | Purpose |
|------|---------|
| `territory/layers/transition/TransitionLayerCoordinator.ts` | Coordinator â€” manages envelope, selects topology vs legacy path |
...
| `territory/layers/transition/modes/FrontierMorphFillMode.ts` | **BROKEN** â€” legacy OT polygon morph fill |
...
| File | Purpose |
|------|---------|
| `territory/adapters/pixi/PixiTerritoryPresenter.ts` | Top-level PIXI presenter â€” delegates to fill + border sub-presenters |
...
| Frontier topology planner | **Implemented, untested** | `buildFrontierTransitionPlan()` â€” diffs topologies, classifies sections |
| Topology frame sampler | **Implemented, untested** | `sampleTopologyFrame()` â€” derives fills from borders |
...
The unified topology path (`FrontierTopologyPlanner` â†’ `TopologyFrameSampler`) is the architectural target. It is **fully implemented** but **disabled** (`TOPOLOGY_PATH_ENABLED = false`). Enabling it requires:
