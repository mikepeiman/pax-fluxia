# Pax Fluxia â€” Terminology & Jargon

> Canonical definitions for project-wide terms. Referenced from `agent.md` Â§3.2.

| Term | Definition |
|------|-----------|
| **Territory** | Connected same-owner stars and the space within their bounds |
| **Frontier** | Boundary geometry where two territories meet â€” the core term for all territory edges |
| **Front** | A contested section of frontier â€” an active attack/battle zone between two players |
| **Region** | A contiguous area of space owned by one player |
| **Holding** | The sum total of a player's territories across the sector |
| **Sector** | The game map |
| **Star** | A node on the map â€” produces ships, can be conquered |
...
| **Tick** | One game time unit â€” all game actions resolve on the synchronized tick |
...
| **Surge** | Visual attack animation â€” ships rush toward target during combat |

## Architecture Terms

| Term | Definition |
|------|-----------|
| **Ownership Layer** | Who owns what â€” graph-native, from conquest events |
| **Geometry Layer** | Shapes from ownership â€” Voronoi cells, regions, frontiers |
| **Transition Layer** | Animating between geometry states â€” morphers, easing |
| **Presentation Layer** | Drawing to screen â€” PIXI.Graphics fills, strokes |
| **Compiler** | Produces geometry data from ownership state â€” no rendering, no PIXI |
