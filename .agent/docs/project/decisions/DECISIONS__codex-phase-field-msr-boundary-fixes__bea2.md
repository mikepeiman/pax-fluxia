...
## Enforcement
- `.agent/memory/collect-dont-rewrite.md` â€” never remove animation options
- `.agent/memory/expose-tuning-variables.md` â€” always expose params to UI
...
## Context
Client and server engines diverged â€” client had rich game logic (overflow accumulation, star type multipliers, pinning penalty, conquest), server had minimal stubs. This caused MP combat regression, missing combat logs, and config mismatches.
...
## Phases
1. âœ… **Math Parity** â€” Production & Repair (completed)
2. â¬œ **Combat & Conquest** â€” Conquest/retreat/scatter logic
3. â¬œ **Cleanup** â€” Strip client engine, full delegation
...
# Decision: Ship Transfer Animation â€” Unified Lifecycle
...
1. **Depart**: Per-frame lerp with tiny factor â†’ ship barely moves, then SNAPS to lane start. "Ships peel off orbit and disappear."
2. **Travel**: Alpha fades in (first 20%) and out (last 20%). "Mere pulses along the lane" â€” ships flash instead of streaming.
3. **Arrive**: Ship goes directly to `orbiting` at `scale: 0.1`. "Ships poof out of star center."

## Decision
- **Unified lifecycle**: Each visual ship transitions through `orbiting â†’ departing â†’ traveling â†’ arriving â†’ orbiting`
- **Same entity**: The visual ship that departs orbit IS the ship that travels the lane IS the ship that arrives
- **Lane adherence**: Ships follow the connection line between stars with slight organic variation (Â±8px perpendicular offset per ship, fading at endpoints)
- **Magnetic easing**: Destination planet "pulls" ships toward it.
  - Depart: `easeInCubic` â€” reluctant departure (slow peel from orbit, then accelerate toward lane)
  - Travel: `easeInCubic` â€” starts slow, accelerates toward target (magnetic pull)
  - Arrive: per-frame orbit lerp â€” ship placed at lane end, lerp smoothly pulls into orbit slot (no bounce)
...
- **Natural arrival**: No dedicated arriving phase. Ship transitions to orbiting at lane end position, and the orbit lerp continuously updates the target as the orbit rotates â€” creating a smooth spiral into orbit.
...
## Context
A `clearTarget` guard auto-cancelled move orders when a star had 0 ships. This broke flow topology â€” the whole game is about chains of command. An empty star should continue forwarding ships when reinforcements arrive.
...
## Decision
- Combat damage transitions active â†’ damaged & active â†’ destroyed (moderated ratio via "Lethality" variable (which needs a better name)), but never damaged â†’ destroyed*.
...
- Visual animations (orbitâ†’departâ†’travelâ†’arrive) fill the inter-tick period for human comprehension.
...
## Decision
- **Active retreat ordered**: `captureRate = 0.35` â€” defender preserves more ships by fleeing
- **Passive loss (no retreat)**: `captureRate = 0.70` â€” more ships captured by victor
...
- **Effective value** = `globalBase Ã— starTypeMultiplier`
- Example: Blue star transfer rate = `0.1 Ã— 2 = 0.2` (Blue has `speed = 2`)
- Same principle for all star types: Yellow 2Ã— production, Red 2Ã— defense, Green 2Ã— attack, Purple 2Ã— repair
- Future (Pax Fluxia roadmap): additional multipliers from star upgrades possible

---

# Decision: Unified Game Settings â€” SP and MP

**Date:** 2026-02-09
**Status:** Active

## Context
SP MainMenu had full game settings (stars/player, ships/star, spacing, links). MP lobby only sent `playerCount`/`mapType`. Server `initStandardMap()` used hardcoded values (`starsPerPlayer = 5`, `minSpacing = 120`). Two completely separate map generators â€” a DRY violation.
...
# Decision: ONE GAME â€” No SP/MP Divergence
...
# Decision: Remove Tone.js â€” Use Web Audio API Directly (or No Audio)
...
## Decision
- **Remove Tone.js entirely** â€” uninstall the package and delete `AudioManager.ts`
...
## Context
At conquest time, `conquest.ts:160` sets `defender.damagedShips = 0` â€” all damaged ships vanish. User observed 50 damaged â†’ 0 on capture. Currently no configurability for what happens to damaged ships.

## Decision
- Add configurable percentage for damaged ships at capture: what % are repaired instantly, destroyed, or captured as-damaged
- **Default**: all damaged ships are captured (as damaged) by the conquering player â€” no instant repair, no destruction
...
- Switched from `Math.random()` to round-robin for guaranteed Â±1 balance
...
## Context
Star types have defined `attack` and `defense` multipliers in `STAR_TYPE_STATS` (Green=2Ã— attack, Red=2Ã— defense) but these were never applied in the combat calculation. Combat used raw ship counts only.
...
> [!WARNING]
> This logic is currently duplicated in both `common/src/engine/GameEngine.ts` and `pax-fluxia/src/lib/engine/GameEngine.ts`. The core refactor to unify to a single engine is pending â€” this duplication is tech debt, not target architecture.
...
- Animation system: user reports current feel is "not quite right" â€” further iteration needed
...
**Date:** 2026-02-11
**Status:** Approved â€” Implementation Pending
...
| **ParticleContainer** â­ | ~200 FPS | 3-4h | Low |
...
# Decision: Conquest Transfer Ships â€” Separate From Normal inFlightToStar
...
# Decision: CONQUEST_TRAVEL_SPEED â€” Intuitive Direction (>1 = faster)
...
# Decision: Transfer Rate â€” Duplication Warning
...
1. `EngineConfig.TRANSFER_RATE = 0.1` (in `common/src/config.ts`) â€” used by `GameEngine.ts`
2. `ORDER_CONFIG.TRANSFER_RATE = 0.25` (in `common/src/orders.ts`) â€” used by `executeTransfer()`
3. Per-star-type `transferRate` (in `STAR_TYPE_STATS`) â€” multiplier on base rate
...
# Decision: Variable Descriptions â€” Plain English Required
...
- Lethality â†’ "Percentage of damaged ships destroyed per attack tick"
- Conquest Transfer % â†’ "Percentage of victor's ships that immediately transfer to conquered star"
- All variables must have tooltips/subtitles in the UI panel

---

# Decision: Engine Unification â€” Atlas-First Approach
...
- **6-phase migration**: Combat formula â†’ Transfer rate â†’ Combat delegation â†’ Full tick delegation â†’ Map gen â†’ Config sync
- **Client keeps**: tick loop/timing, AI evaluation, combat logging, history, animation hooks

## Reference Docs
- `ENGINE_ARCHITECTURE_CURRENT.md` â€” current duplication inventory
- `ENGINE_ARCHITECTURE_TARGET.md` â€” desired unified state
...
## Context
Colyseus WebSocket connections fail with "seat reservation expired" (close code 4002) when the server runs under Bun's runtime. The `@colyseus/ws-transport` depends on the `ws` library (Node.js WebSocket implementation). Bun's runtime replaces Node's `http` module internals with its own implementation, which is incompatible with how `ws` handles the HTTP `upgrade` event for WebSocket connections. This was discovered locally first â€” `bun --watch src/index.ts` failed, but `npx tsx --watch src/index.ts` (Node runtime) worked.
...
# Decision: Colyseus Module Resolution â€” No Explicit WebSocketTransport Import

**Date:** 2026-02-14
**Status:** Active (CRITICAL)

## Context
Explicit `import { WebSocketTransport } from "@colyseus/ws-transport"` in `prod.ts` caused bun's content-addressable node_modules to load **two separate `@colyseus/core` instances** â€” each with its own `matchMaker` singleton and `rooms` map. Rooms were created in one map but looked up from the other, causing "seat reservation expired" (4002).
...
# Decision: Settings Panel â€” "Economy" Is "Global Settings" + Mirrored Duplicates OK
...
## Decision
1. **"Economy" is the wrong label** â†’ rename to **"Global Settings"**
2. **Attack/Defense sliders belong in Global** â€” they are global modifiers, not economy-specific
3. **Variables MAY be duplicated across panels** under two conditions:
   - Labels must be **exactly the same** in both panels
   - Values must be **bound together as mirrors** â€” changing one updates the other in real-time
4. Current problem: labels differ ("Defense" â‰  "Aggressor Advantage") and values are not bound

## Action Items
- Rename "Economy" section â†’ "Global Settings"
...
# D-78: Conquest Animation â€” Localized Frontier Updates
...
1. **Anchor points**: Where the affected frontier meets unaffected geometry â€” the geometry beyond these points does not move
...
5. **Performance note**: Chaikin is O(n) per pass. With ~128 vertices and 2-3 passes at 60fps, this is ~23k array ops/sec per segment â€” trivial
...
# D-79: Territory Fill Morph â€” Shape, Not Crossfade
...
- Old territory shape fades out at `alpha Ã— (1-t)`
- New territory shape fades in at `alpha Ã— t`
...
# D-80: Unified Frontier Pipeline â€” Point-Line Canonical Data
...
Pipeline:
1. d3-weighted-voronoi â†’ raw cells
2. Merge same-owner cells â†’ closed polygons (`MergedTerritory`)
3. Chaikin smoothing (junction + boundary pinning) â€” unchanged
...
- Polygon rotation alignment (`alignClosedPolygon`) â€” minimizes total vertex displacement
...
## Context
The PRD defined a 6-layer model (Truth â†’ Frontier â†’ Fitting â†’ Region â†’ Presentation â†’ Transition). Code review revealed that layers 2-4 are sub-steps of one concern ("turn ownership into shapes"), while the live code in `renderMode.ts` already has a cleaner 3-concern contract.

## Decision
Adopt 4-layer model:
1. **Ownership** â€” "Who owns what" (`territory/ownership/`, `GraphOwnershipState`)
2. **Geometry** â€” "What shapes exist" (`territory/geometry/`)
3. **Transition** â€” "How shapes change between ticks" (`territory/transitions/`, as FX handlers)
4. **Presentation** â€” "How shapes become pixels" (`territory/render/`)
...
# Decision: Territory Engine â†’ TerritoryOrchestrator
...
## Decision
Rename `territory-engine/` to `territory/orchestrator/` and rename `engine.ts` concepts to "orchestrator" â€” this better describes its role as a route-and-dispatch coordinator, not a compute engine.
...
## Decision
Refactoring the territory renderer uses a **dual-adapter approach**: a new `refactored_pvv2` adapter runs alongside the untouched `legacy_pvv2`. New registry entries (`FG1 Mar19 Refactor`, `DY4 Mar19 Refactor`) appear in the Mode dropdown, letting the user switch between working original and refactored code at runtime. Original `legacy_pvv2` path is **never modified** â€” zero risk to DY4 SACROSANCT animation.
...
## Key Trace Finding
At 4 px the dominant cost is the **per-transition Power-Voronoi PREV-geometry rebuild** inside `MetaballGridFamily.buildPlanForTransition` (45.8 % of trace time) â€” not per-frame paint. This was flagged as a known simplification in the family file header. Phase C promotes that checkpoint to blocking.
...
- **Phase B (1-2 d):** two-layer caching â€” static RenderTexture for natives + dirty-rect blit on ownership change + dynamic overlay for dispossessed cells during transitions. Target: 100 k+ cells @ 60 fps on iGPU.
...
- **Phase D (stretch, 1-5 d):** pick one â€” splat-and-threshold metaballs (real soft blobs, â‰¤50 k splats) OR JFA territory field (resolution-bound, decouples from N).
...
- Static RenderTexture GPU memory = `worldWidth Ã— worldHeight Ã— 4 bytes`; Â½-res fallback may be needed for world > 4000 px.
...
### CP â€” Contested-lane midpoint Pair
...
- **Stage:** pre-metaball, same pipeline. Conditional by design â€” on many maps legitimately produces nothing (no owner has â‰¥ 2 disconnected components).
...
### MSR â€” Minimum Star Range
- **Expansion:** *margin around a star within which lanes that do not originate at that star should not pass.*
- **Current implementation:** power-diagram site-weight term (`MODIFIED_VORONOI_STAR_MARGIN`, internally squared) in `powerVoronoiTerritoryGeometryGenerator.ts` L110-125. Not a hard "push geometry inward" stage â€” a weighting nudge. Can feel weak or ambiguous rather than presenting as a clean visible moat.
...
- **Consume pre-shaped polygons â†’ get CX/CP/DX/MSR for free:** metaball-grid, perimeter-field, any future polygon-consuming family.
...
3. **New queued task â€” MG-MSR-LANE-FILTER:** implement the missing lane-level MSR filter as a separate pass in lane generation, independent of the Voronoi site weight. Queued in today's daily queue; not scheduled yet.
4. **New queued task â€” MG-CP-SHORT-LANE-AUDIT:** verify CP emission on short lanes + the metaball-family wiring gap. Queued; not scheduled yet.
