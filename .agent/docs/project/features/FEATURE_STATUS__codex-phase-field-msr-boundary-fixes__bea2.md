...
| â“ | Agent-implemented, not user-verified |
| âŒ | User-confirmed broken |
...
|----|---------|--------|-------|
| C-1 | Start Game | âœ… | Menu â†’ Game transition |
| C-2 | Pause/Resume | â“ | Orbit + traveling ship freeze added (`60395be`) |
| C-3 | Speed Control (1x-50x) | âœ… | |
| C-4 | Win/Lose Detection | âœ… | |
| C-5 | Replay / Return to Menu | âœ… | SP only. MP restart broken (see B-18) |
| C-6 | Game Over Screen | âŒ | No Game Over screen on full conquest in MP. Need modal: View Results or Keep Playing |
...
| ID | Feature | Status | Notes |
|----|---------|--------|-------|
| I-1 | Click-to-Select Star | âœ… | |
| I-2 | Drag-to-Attack | âœ… | |
| I-3 | Click-Click Attack | âœ… | |
| I-4 | Right-Click Cancel | âœ… | |
| I-5 | Command Issue Speed | â“ | Uses `pendingOrders` for optimistic UI |
| I-6 | Vector Arrow Display | â“ | Uses `pendingOrders` for instant rendering |
...
| ID | Feature | Status | Notes |
|----|---------|--------|-------|
| M-1 | Attrition (per-tick damage) | â“ | V4 symmetric formula. UI sliders wired via `CombatConfigOverride` (`441010d`) |
| M-2 | Conquest (star capture) | â“ | Per-player aggregation via `shipsByOwner` map (`43cd2fc`) |
| M-3 | Conquest Threshold | âœ… | User verified 2026-02-08 |
| M-4 | Retreat (directed) | â“ | 35% capture rate. Needs user verification |
| M-5 | Scatter (escape routes) | â“ | Uses star connections. Needs user verification |
| M-6 | Ship Transfer Rate | âœ… | Slider step changed to 1% (`441010d`) |
| M-7 | Repair (damaged â†’ active) | âœ… | User verified 2026-02-08 |
| M-8 | Production (ship spawn) | âœ… | |
| M-9 | Order Persistence | â“ | Orders persist until cancelled; `clearTarget` guard removed |
| M-10 | Order Cancel on 3rd-Party Conquest | â“ | Chained orders also cancelled (`60395be`) |
...
| ID | Feature | Status | Notes |
|----|---------|--------|-------|
| V-1 | Ship Orbit (idle) | âœ… | |
| V-2 | Ship Transfer (flow along lanes) | â“ | Unified lifecycle: orbitâ†’departâ†’travelâ†’arrive |
| V-3 | Tick Orb Pulse | âœ… | |
| V-4 | Star Glow / Selection | âœ… | |
| V-5 | Facing Departure (orbit dance) | â“ | Toggle in Animation Tuning, OFF by default. Sort+splice causes dance effect |
| V-6 | Orbit Bias Oscillation | â“ | Min/Max/Freq controls. Oscillates bias strength per tick frequency |
| V-7 | Smooth Departure Easing | â“ | easeInOutQuad replaces easeInCubic for smoother departure |
| V-8 | Metaball blur targets | â“ | `METABALL_BLUR_AFFECTS_BORDERS` toggle: default blur fill only; optional single-pass blur on fill + border strokes when GPU blur is on. |
| V-9 | Mapgen lane clearance + CX | â“ | Delaunay prune and lane polylines use **`MAPGEN_LANE_MARGIN_PX`** only; **MSR** = territory boundaries; curved mode is necessity-only (no cosmetic bulge); live Map & Grid; CX via `buildCorridorVirtualSites`. **Queued:** cross-player **vstar near lane midpoint** per owner to block enemy territory overlap. |
| V-10 | Metaball same-owner overlap banding | â“ | Influence is **summed per player** (`infGeom[p] += c`), so two stars of one owner **double** the field in overlap â€” can read as seams/artifacts vs a single smooth blob. **Fix:** moderate effort â€” e.g. take **per-cluster max** instead of sum for real stars, or normalize by local star count; needs visual QA so territory area does not shrink unexpectedly. |
...
|----|--------|--------|-------|
| P-1 | Transfer Rate | âœ… | Step=1%, min=1%. Writes `GAME_CONFIG.TRANSFER_RATE` (Ã·100) |
| P-2 | Aggressor Advantage | â“ | Wired via `CombatConfigOverride` (`441010d`) |
| P-3 | Damage Per Ship | â“ | Wired via `CombatConfigOverride` (`441010d`) |
| P-4 | Lethality | â“ | Wired via `CombatConfigOverride` (`441010d`) |
| P-5 | Force Ratio Effect | â“ | Wired via `CombatConfigOverride` (`441010d`) |
| P-6 | Conquest Threshold | âœ… | Reads directly from `GAME_CONFIG` in GameEngine |
| P-7 | Conquest Transfer % | â“ | Reads from `GAME_CONFIG` |
| P-8 | Retreat Capture Rate | â“ | Reads from `GAME_CONFIG` |
| P-9 | Scatter Capture Rate | â“ | Reads from `GAME_CONFIG` |
| P-10 | Scatter Destroy Rate | â“ | Reads from `GAME_CONFIG` |
| P-11 | Damaged Ship Effectiveness | â“ | Reads from `GAME_CONFIG` |
| P-12 | Repair Rate | â“ | Reads from `GAME_CONFIG` |
| P-13 | AI Attack Threshold | â“ | Reads from `GAME_CONFIG` in `AI.ts` |
| P-14 | AI Desist Threshold | â“ | Reads from `GAME_CONFIG` in `AI.ts` |
| P-15 | AI Random Aggression | â“ | Reads from `GAME_CONFIG` in `AI.ts` |
| P-16 | AI Tactical Aggression | â“ | Reads from `GAME_CONFIG` in `AI.ts` |
| P-17 | Arrow Length | â“ | `ARROW_LENGTH_FRACTION` (10-100%), default 50% (`d38cdba`) |
| P-18 | Facing Departure Toggle | â“ | `FACING_DEPARTURE` checkbox in Animation Tuning |
| P-19 | Oscillate Bias Toggle | â“ | `ORBIT_BIAS_OSCILLATE` checkbox + Min/Max/Freq sliders |
| P-20 | Global Production | â“ | `BASE_PRODUCTION` slider (0-3) in Global Bonuses section |
| P-21 | Global Repair | â“ | `REPAIR_RATE` slider (0-1) in Global Bonuses section |
| P-22 | Global Defense | â“ | `AGGRESSOR_ADVANTAGE` slider (0.1-2) in Global Bonuses section |
| P-23 | Global Attack | â“ | `DAMAGE_PER_SHIP` slider (0-0.5) in Global Bonuses section |
...
| B-29 | ~~Pause/play freezes attack surge~~ **FIXED**: tickProgress now uses `BASE_TICK_MS` instead of `ANIMATION_SPEED_MS` â€” eliminates dead zone where `sin(Ï€)=0`. |
...
| B-6 | Transfer rate equilibrium | `Math.ceil` + `TRANSFER_RATE Ã— star-type speed` | `e42320c` |
...
| B-69 | ~~**Garbled unicode in MainMenu.svelte**~~ **FIXED**: UTF-8 box-drawing chars `â”€` rendered as mojibake `â‚¬Ã¢"â‚¬`. Replaced with ASCII dashes. | 2026-02-17 |
...
| F-21 | Tauri Desktop Build: native .exe, .msi, .nsis installer (1400Ã—900 default window) | 2026-02-12 |
...
| F-24 | Conquest Lerp Delay (200ms) + Slowmo Mode (4Ã—/10Ã— toggle, full framerate) | 2026-02-12 |
...
| F-28 | Ship Density VFX: HSL-based color graduation for high ship counts. 4Â° hue shift Ã—3 tiers each direction, sat+lightness in opposing directions. No size changes. Replaces old white-wash hue-brighten. | 2026-02-14 |
...
| ID | Feature | Priority |
|----|---------|----------|
| R-1 | AI: Frontline Forces | ðŸŸ¢ |
| R-2 | AI: Match Opposing Forces | ðŸŸ¢ |
| R-3 | AI: Evenly-Distributed | ðŸŸ¢ |
| R-4 | AI: Backline-and-Pounce | ðŸŸ¢ |
| R-5 | AI: Tactical Surround | ðŸŸ¢ |
| R-6 | AI: Star Type Awareness | ðŸŸ¢ |
| R-7 | AI: Pre-Conquest Retreat | ðŸŸ¢ |
| R-8 | Conquest Stats Popup (pause + stats card) | ðŸŸ¢ |
| R-9 | Slow-Mo Debug Mode | ðŸŸ¢ |
| R-10 | Animation Style Toggle | ðŸŸ¢ |
| R-11 | Ship Color Fade (mid-travel) | ðŸŸ¢ |
| R-12 | Damaged Ship Overlapping Orbits | ðŸŸ¢ |
| R-13 | Scrollwheel Zoom | ðŸŸ¡ |
| R-14 | ~~Performance Audit~~ â†’ see R-114 through R-118 | âœ… AUDITED 2026-02-15 |
| R-114 | **Spatial Culling: Ship Orbits**: Skip orbit physics + drawShip for off-screen stars. Still track spawn/despawn counts. Needs viewport.ts utility (getViewportBounds, isStarVisible) | ðŸ”´ |
| R-115 | **Spatial Culling: Traveling Ships**: Run lifecycle (arrivals must happen) but skip drawShip when ship position off-screen | ðŸ”´ |
| R-116 | **Spatial Culling: Stars/Labels/Glow**: Set graphics.visible=false for off-screen stars instead of redrawing every frame | ðŸ”´ |
| R-117 | **In-Flight Map Optimization**: renderShips scans ALL travelingShips per star O(starsÃ—ships) â€” pre-build Map\<starId, count\> in O(n) | ðŸ”´ |
| R-118 | **starsById.get() Fix**: renderShips uses stars.find() for targetStar lookup â€” should use existing starsById Map | ðŸ”´ |
| R-15 | ~~Multiplayer Deployment~~ | âœ… RESOLVED 2026-02-14 â€” dual module instance fix |
| V-3 | Territory Alpha Masks (Voronoi/gradient ownership overlays) | ðŸ”´ |
| V-4 | Travel Animation Polish (elegant, smooth, satisfying feel) | ðŸ”´ |
| R-16 | Percentage Directives | ðŸ”µ |
| R-17 | Command Arrow Styles | ðŸ”µ |
| R-18 | Custom Map Editor | ðŸ”µ |
| R-19 | Imperative Animation Events (architectural) | ðŸ”´ |
| R-20 | Star Upgrades (spend ships to upgrade) | ðŸ”µ |
| R-21 | Star Hybrids / Multi-classing | ðŸ”µ |
| R-22 | Spectator Mode (MP) | ðŸŸ¡ |
| R-23 | Take Over AI Player (MP) | ðŸŸ¡ |
| R-24 | Vote System for MP Settings | ðŸŸ¡ |
| R-25 | Multiple Targets / Branching Orders | ðŸ”µ |
| R-26 | Conditional Orders (trigger on conquest) | ðŸ”µ |
| R-35 | Panel Reorganization: Separate "Ship Movement" (orbits + travel to friendly destinations) and "Combat & Conquest" (attack surge, conquest transfer, capture, scatter, retreat) into distinct control panel sections | ðŸŸ¢ |
| R-27 | Strategic Patterns (auto-behavior algorithms) | ðŸ”µ |
| R-28 | Tailwind CSS Migration (70-80% inline utility, CSS custom properties for JS interpolation) | ðŸŸ¡ |
| R-29 | Last-Stand Resistance Mode | ðŸŸ¢ |
| R-30 | Captured Ship Activation Rate Slider | ðŸŸ¢ |
| R-31 | Deferred Order Arrows Overlay Priority | ðŸŸ¢ |
| R-32 | **Public Room Browser** â€” list available rooms in MP tab, click-to-join with confirmation | ðŸŸ¢ |
| R-33 | **Per-Player Settings** â€” color, AI strength/strategy, human handicaps/buffs in main menu | ðŸŸ¢ |
| R-94 | Visual Style Packs (selectable animation & visual presets, gameplay feel variety) | ðŸ”´ |
| R-133 | Tick Length Control Panel Slider | ðŸŸ¢ |
| R-134 | End-Game Screen Enlargement + Better Charts | ðŸŸ¡ |
| R-135 | Damaged Ships Never Destroyed (design rule â€” see MECHANICS Â§6) | ðŸŸ¢ |
| R-34 | AST-Based Bidirectional Documentation | ðŸ”µ |
| R-35 | Conquest Pause + Stats Card Popup | ðŸŸ¢ |
| R-36 | Damaged Ship Visual Density Tiers (overlapping orbits) | ðŸŸ¢ |
| R-37 | Full Engine Unification: server uses shared GameEngine for map generation | ðŸ”´ |
| R-38 | **Main Menu Restructure** â€” always-visible room browser (no SP/MP tabs), unified layout | ðŸŸ¢ |
| R-39 | **Standard Menu Items** â€” Gameplay Options, Settings (audio/video icons), Shop, Quit to Desktop | ðŸŸ¢ |
| R-136 | Ship Orbit Density: limit to 5 layers, scale colors max 5000 ships, enlarge on overflow | ðŸ”´ |
| R-137 | Power Density VFX: animations/effects for increasing fleet power at stars | ðŸ”´ |
| R-40 | Leaderboard: emphasize TOTAL SHIPS first, then active/damaged fraction | ðŸŸ¢ |
| R-41 | Surrender Modal: End Game (results) or Abandon (main menu) â€” replaces raw button | ðŸŸ¢ |
| R-42 | Player Stats Console: bottom-up drawer replacing combat log, shows live dynamics | ðŸ”´ |
| R-43 | SP/MP Full Parity: ONE GAME â€” eliminate all divergent UI, logic, and settings paths | ðŸ”´ |
| R-44 | AI Difficulty Levels + Strategy/Posture Options (clearly specified behaviors) | ðŸ”´ |
| R-45 | Star Spacing Formula: min = diameter + 5 orbits + adjustable buffer (e.g. 20px) | ðŸŸ¢ |
| R-46 | Orbit Density Controls: exposed in-game variable for orbit layer/scale tuning | ðŸŸ¢ |
| R-47 | Per-Target Conquest Transfer Amount: player sets per-target override superseding global | ðŸŸ¡ |
| R-48 | Scatter Visual Effects: scatter events need distinct visual presence â€” ships breaking away to escape routes | ðŸ”´ |
| R-49 | Capture Animation: text enlarges + damaged ships animate conversion to friendly â€” creative VFX proportional to captured count | ðŸ”´ |
| R-50 | Oscillation Period Slider: expose orbit bias oscillation period/frequency as a range slider in Animation Tuning | ðŸŸ¢ |
| R-51 | Orb Travel Animation: ships merge into glowing orb during travel, fragment on arrival â€” orb brilliance proportional to ship count | ðŸŸ¢ |
| R-52 | End Game Screen Redesign: fullscreen 900px modal, ambient glow, scoreboard, enlarged charts, shimmer button, premium VFX | ðŸŸ¢ |
| R-53 | Gameboard Resize-to-Fit: hotkey/button to resize gameboard to fit available viewport space | ðŸ”´ |
| R-54 | Orb Travel Tuning: full set of sliders for orb colors, gradients, glow, sizes â€” all persisted to localStorage | ðŸŸ¡ |
| R-55 | Scatter/Retreat Animation Type: separate scatter/retreat from movement animations â€” distinct visual language for fleeing ships | ðŸ”´ |
| R-56 | Scatter Escape-Capture Mechanic: when ships scatter, some may be captured by nearby enemy stars â€” escape routes determined by proximity | ðŸ”´ |
| R-57 | Elimination Screen: show modal when player eliminated â€” Spectate or End Game options | ðŸŸ¢ |
| R-58 | Nearside Departure/Arrival: ships depart from side facing target, arrive on nearside, outer-layer-first departure selection | ðŸŸ¢ |
| R-59 | Orb Fragmentation Boundary: orbs break apart outside outermost orbit ring, ships fly directly to orbit slots with eased curve | ðŸŸ¢ |
| R-60 | Time-based Arc Settle: ships orbit-snap via easeOutCubic over SETTLE_DURATION_MS (150ms default), never cross star center â€” polar interpolation | ðŸŸ¢ |
| R-61 | Crisp Text at Zoom: all PIXI.Text rendered at 2x resolution for sharp text at any zoom level | ðŸŸ¢ |
| R-62 | No-Cross-Star Constraint: ships always arc around star perimeter to reach orbit slot, never path through center | ðŸŸ¢ |
| R-63 | Depart Mode Selector: dropdown for LIFO/FIFO/Nearside departure selection with panel persistence | ðŸŸ¢ |
| R-64 | Settle Duration Slider: exposed SETTLE_DURATION_MS (30-500ms) in panel for tuning orbit snap speed | ðŸŸ¢ |
| R-65 | Nearside Departure Fix: dot product now computed against orbit SLOT position (getOrbitSlot) instead of mid-settle ship.x/y | ðŸŸ¢ |
| R-66 | Arrival Stagger: ships settle across tickÃ—ARRIVAL_SPREAD (0-2x slider), spawn/repair/transfer all staggered | ðŸŸ¢ |
| R-67 | Wobble Travel: sinusoidal perpendicular oscillation on travel paths, per-ship frequency/phase, fades at endpoints (0-40px slider) | ðŸŸ¢ |
| R-68 | Settle Slider Extended: range expanded to 0-2000ms for full range from instant to slow drift | ðŸŸ¢ |
| R-69 | Sprite Pool Ship Rendering: replaced per-ship Graphics.circle with batched sprite pool for O(1) draw calls | ðŸŸ¢ |
| R-70 | FPS + Ship Count Overlay: real-time FPS, total visual ships, and sprite pool size displayed in monospace badge | ðŸŸ¢ |
| R-71 | Spawn/Production Instant Settle: removed ARRIVAL_SPREAD stagger from spawned ships â€” only travel arrivals stagger | ðŸŸ¢ |
| R-72 | Sharper Sprites: circle texture increased 16â†’64px with anti-aliased edge for crisp rendering at all scales | ðŸŸ¢ |
| R-73 | Reset All Button: CombatDebugPanel has red-tinted reset button that clears localStorage and restores defaults | ðŸŸ¢ |
| R-74 | Orbit Density Slider: controls ship spacing per ring (1.0-4.0x), higher = more spread out, fewer per ring | ðŸŸ¢ |
| R-75 | ~~Attack Surge Slider~~ â†’ moved to F-19 (verified working) | âœ… |
| R-76 | ParticleContainer Rendering: replaced Graphics.circle with ParticleContainer + Particle pool (24fps @ 30k ships) | ðŸŸ¢ |
| R-77 | Player-Color Outlines: ring texture behind each ship shows raw player color, visible even when fill is white-blended | ðŸŸ¢ |
| R-78 | Orb Travel Flash Fix: departing ships fade to full transparency before orb grouping, eliminating single-frame arc | ðŸŸ¢ |
| R-79 | Stars Panel Sorting: group/sort by owner, ship count, or star name â€” dropdown in Stars panel header | ðŸŸ¢ |
| R-80 | Bottom-Drawer Combat Log Panel: short bottom drawer (~6-8 lines), styled per-line combat/conquest events, full formula on one line, replaces old Combat Logs panel | ðŸ”´ |
| R-81 | Ship Density Color Graduation: HSL-based auxiliary colors (3 per side, 4Â° hue shift each, sat/lightness in opposing directions) replacing white-wash for high ship counts | âœ… |
| R-82 | Timing Section: rename Game Speed â†’ TIMING, consolidate ATTACK_SURGE_RAMP_MS + CONQUEST_LERP_DELAY_MS sliders | ðŸŸ¢ |
| R-83 | AI Pinning Strategy: intelligent pinning that ignores mere force ratio between two stars â€” sophisticated movement | ðŸ”´ |
| R-84 | AI Multi-Source Attacks: lower AI considers single star ratio, smarter AI considers all available sources | ðŸ”´ |
| R-85 | Conquest Visual Flair: border and background do several transitions â€” lightness, size, glow. Border should shine/pulse. Star icon should brighten + expand-contract like a single heartbeat | ðŸ”´ |
| R-86 | Ship Travel Spread: ships currently travel as "vertical walls/slices" perpendicular to lane. Need more spread in both dimensions, individual wobbles with smaller-group coherence. See also R-111 | ðŸ”´ |
| R-87 | Easing Curve Visual Editor: lightweight JS widget for standard easing parameters (cubic-bezier style) | ðŸ”´ |
| R-88 | Attack Surge Tick-Sync: surge should only animate during actual combat ticks, not on order issuance | ðŸ”´ |
| R-89 | Unified Game-Start Screen: SP & MP share same lobby/settings UI, eliminate divergent paths | ðŸ”´ |
| R-90 | Per-AI Player Settings: select difficulty + strategy for each AI independently at game start | ðŸ”´ |
| R-91 | AI Custom Personality Editor: create/save/load named AI profiles with tuned strategies/variables | ðŸ”µ |
| R-92 | Game Sounds: tick sound, conquest sound, conquest-ships-arrival sound (lightweight, no Tone.js) | ðŸ”´ |
| R-93 | Animation General Polish: conquest, transfer, and orbit-meshing visual improvements | ðŸ”´ |
| R-95 | Lane Stationing: ships can be stationed in lanes between stars â€” changes travel timing & defense dynamics | ðŸ”µ |
| R-96 | Satellites/Planets/Battle Stations: purchasable orbital structures that orbit stars | ðŸ”µ |
| R-97 | Planetary/Star Shields: defensive shields protecting stars from attack | ðŸ”µ |
| R-98 | Mines: deployable hazards in lanes or around stars | ðŸ”µ |
| R-99 | Economy & Production System: resource economy to support purchasing structures, shields, and mines | ðŸ”µ |
| R-100 | Ownership Inversion: Stars are currently a container for Ships. Future variation: Ships as containers for Stars, or more generally Locations/Roles â€” bidirectional ownership, increased complexity | ðŸ”µ |
| R-101 | **Timing Panel Consolidation**: Tick interval, Animation speed, Travel duration, Settle time in ONE panel. Master slider adjusts all at once. Independent control via toggle. Numeric inputs showing ratio with tick duration (0-5, step 0.25) | ðŸ”´ |
| R-102 | **HSLA Ship Accent Controls**: Controls that modify player primary color for outline/glow accents | ðŸŸ¢ |
| R-103 | **Ship Glow Effect**: Actual glow rendering for ships | ðŸŸ¢ |
| R-104 | **Lane-Star Blending**: Attach lane ends visually to stars for cleaner look | ðŸŸ¢ |
| R-105 | **Victory Conditions System**: Pre-game setting category. Classic Mode = 100% conquest. Pax Fluxia offers variable thresholds by ship count, specific star/territory conditions | ðŸ”´ |
| R-106 | **Rapid-Fire Sequential Departures**: Ship transfers as steady stream â€” single ships in rapid-fire sequence, randomly-eased arrivals | ðŸŸ¢ |
| R-107 | **Arrival Centerline Targeting**: Arrivals head to lane centerpoint (not lane-star boundary intersection). Variable scatter-offset from centerline. Arc to orbit slot within same easing function | ðŸŸ¢ |
| R-108 | **Travel Time Game Mode (1x-10x)**: Variable travel duration changes gameplay dynamics. Ships exist as fleets at intermediate points. Below 1x = cosmetic only. Above 1x = mechanical. Opposing fleets on same lane either pass or fight | ðŸ”µ |
| R-109 | **Lane Combat (fleet interception)**: When travel time > 1x, opposing fleets on same lane fight en-route. Branch: pass unaffected OR fight each other | ðŸ”µ |
| R-110 | **Ship Power Density Visual**: HSL-based density tiers in drawShip + 4 config handles (DENSITY_HUE_STEP, DENSITY_SAT_STEP, DENSITY_LIGHT_STEP, DENSITY_TIERS) + debug slider 0-10k. *See also R-39.* | âœ… |
| R-111 | **Ship Travel Animation Handles**: More controls for flight behavior â€” spread in both dimensions, individual wobbles, smaller-group coherence | ðŸŸ¢ |
| R-112 | **Slider Detent at Mid**: UI sliders should have a small notch/tag at midpoint that can be clicked to center the value | ðŸŸ¡ |
| R-113 | **Animation Speed = Tick Duration Ratio**: Animation speed bound to tick duration. Control = how much of a tick duration it lasts (0-1, default 0.5) | ðŸŸ¢ |
| R-119 | **Configurable Win Conditions**: `GAME_WIN_DETERMINANT` = [ships, stars, targets] (win by ship count, star count, or specified target ownership). `GAME_WIN_THRESHOLD` (default 95% ships). Modal: View Results or Keep Playing (continues to full-map conquest). Game option in pre-game settings. | ðŸ”´ |
| R-120 | **Color Adjustment Toggle**: toggle between adjusting player colors independently vs locking current hue offset and adjusting all together | ðŸŸ¢ |
| R-121 | **Color Merge on Star Capture** (idea: Kaya): When you capture a star, you merge colors with the defeated enemy â€” triggers effects: (1) spawns a new AI player of that merged color, (2) captured star becomes hybrid/mixed color. Evocative of Elemental thinking where each color is fundamentally distinct. | ðŸ”µ |
| R-122 | **Custom Game Themes**: saveable/loadable theme presets (visual style, color palette, game settings) persisted to localStorage. Users can name, save, switch between themes. | ðŸŸ¡ |
| R-123 | **Community Content Hub**: project directory for storing/sharing custom themes and maps. Three tiers: official maps (from dev), public user-contributed maps, and user themes. Infrastructure for future sharing/import/export of rich game config presets. | ðŸ”µ |
| R-124 | **Spectator Mode on Mid-Game Join**: New players joining in-progress games enter as spectators (observers). Can choose to take over an AI player, or request takeover of a human player (with consent). UI shows "Spectating" player list. | ðŸ”´ |
| R-125 | **Conquest Reactivation Delay**: Newly conquered stars have a cooldown before production/orders activate. Config: `CONQUEST_REACTIVATION_ENABLED` (bool), `CONQUEST_REACTIVATION_TICKS` (int, default 2-3). Adds strategic cost to conquest â€” prevents instant counter-attack from captured star. | ðŸŸ¢ |
| R-126 | **Category Theme File Persistence**: Category theme save/load should write to project files (not only localStorage), immediately surfaced as options, persisting beyond reloads/localStorage wipes. Every panel needs reset-to-defaults (scoped to category). CategoryThemeBar already has save/load UI â€” investigate `categoryThemes.ts` storage backend. | ðŸ”´ |
| R-127 | **MSR/CX/DX Geometry Model Alignment**: New geometry model should handle MSR, CX, DX constraints inherently, but these toggles/values still have effect in updated render modes. Architecture misalignment â€” investigate which legacy constraint toggles remain active when they shouldn't be. | ðŸ”´ |
| R-128 | **Portal Stars**: In classic Pax Galaxia maps, numeric star types (`2`, `3`, etc.) are portal stars. All instances of the same numeric type are simultaneously occupied by whoever captures one â€” bridging graph-disconnected regions. Found in: Boxed, CrissCross, DSpokes, Frontline, Arena, Crazy. **Implemented on 2026-04-22 (partial first slice):** `classic-map-parser.ts` now parses numeric types â†’ `starType='portal'` + `portalGroup`, the shared engine synchronizes ownership across the group on conquest, runtime/server/editor/export paths preserve `portalGroup`, and gameplay/editor/menu thumbnails now render portal stars distinctly. **Still open:** explicit portal-family UI in the main game, richer portal VFX, and broader gameplay design if portals need behavior beyond synchronized occupancy. Reference: "Boxed" (type-2 at x152/y53 â†” x747/y56, type-3 at x748/y489 â†” x155/y487). | â“ |
| R-129 | **Settings Panel Reorganization**: Refactor settings panel groupings for more intuitive layout. Apply high-level thinking about which settings belong together and create best-appropriate, intuitive groupings. Current groups evolved organically â€” reorganize with UX intent. | ðŸ”´ |
| R-130 | **Settings Files Modularization**: Refactor monolithic settings files (settingsDefs.ts, GameSettingsPanel.svelte) into sub-modules. Each panel group should be a properly modular import. Against engineering practices to maintain giant monolithic files. | ðŸ”´ |
...
| B-86 | No attacker attrition: attacking ships not destroyed proportional to received damage Ã— Lethality | Investigating |
...
| F-50 | Split repair suppression | High | â“ Implemented â€” `REPAIR_SUPPRESS_ATTACKER` (0.5) / `REPAIR_SUPPRESS_DEFENDER` (0.1), see `production.ts` |
...
| ID | Feature | Status | Notes |
|----|---------|--------|-------|
| A-1 | Engine Unification: Combat formula | âœ… | `calculateCombatV4` wraps `sharedCalculateCombat` from @pax/common |
| A-2 | Engine Unification: Transfer rate | âœ… | `ORDER_CONFIG` removed as dead code (2026-02-12) |
| A-3 | Engine Unification: Client combat delegation | âœ… | Client GameEngine.ts deleted; common is sole engine |
| A-4 | Engine Unification: Client tick delegation | âœ… | `gameStore` uses `GameEngine.tick()` from common |
| A-5 | Engine Unification: Map generation shared | âœ… | `@pax/common/mapgen` extracted (2026-02-14) |
| A-6 | Engine Unification: Config defaults sync | âœ… | `GAME_CONFIG` proxy auto-persists; overrides passed to engine |
...
| TR-28 | 10Ã— slow-mo ignores user timing | Active | 2026-03-20 |
...
| Date | Summary |
|------|---------|
| 2026-05-02 | **0319 shared geometry authority reset:** Replaced the old raw-adapter plus consumer-local repair pattern with one shared post-0319 authority builder. Live `power_voronoi_0319` consumers now receive a resolved shared-boundary snapshot with explicit raw/resolved/display stage diagnostics, phase-field reads that authority seam instead of re-resolving locally, and `Show Underlying Geometry` / artifact export are stage-selectable instead of ambiguous. |
| 2026-05-01 | **Phase-field pattern-lattice correction:** Fixed the regression where phase-field visible fill was being bucket-sampled from the conquest scheduler grid. The mode now uses a separate cached presentation classification for PRE/POST visible fill, adds a real `Pattern Spacing` control, relabels scheduler spacing as `Transition Spacing`, and restores `Cell Shape` / `Cell Inset` / `Square Corner` as actual visible fill controls instead of sparse dot-field artifacts. |
