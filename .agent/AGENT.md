...
### 2026-05-01 - Restore Live Cell-Shape Tuning In Phase Field

- Lane: `territory/phase-field-cell-shape-cache-fix`
- User task: the entire `Cell Shape` subsection appeared dead while tuning conquest fills. Trace the wiring and restore live behavior.

#### Pass Log

1. Pass 1 - Audited the settings write path in `MetaballGridTuning.svelte` and confirmed the controls were writing:
   - `METABALL_GRID_CELL_SHAPE`
   - `METABALL_GRID_CELL_INSET_PX`
   - `METABALL_GRID_CELL_CORNER_PX`
2. Pass 2 - Traced those tunables into `MetaballGridPhaseFieldFamily.ts` and found the real fault:
   - the phase-field transition mask render path did consume `cellShape`,
   - but `lastMaskTextureSig` did not include primitive-shape state,
   - so circle / diamond / hex could keep reusing an old cached square-shaped mask until some unrelated change invalidated the texture.
3. Pass 3 - Fixed the mask cache signature to include:
   - `cellShape`
   - `transitionHexR`
4. Pass 4 - Left steady-state behavior unchanged:
   - shape controls affect the conquest cell overlay and grid-edge border fallback,
   - they do not restyle the smooth geometry-driven steady-state territory fill.

#### Validation

- `git diff --check`
- filtered `bun run check` produced no new hit for:
  - `MetaballGridPhaseFieldFamily.ts`

#### Merge Note

- Functional conflict surface for this pass is:
  - `pax-fluxia/src/lib/territory/families/metaballGrid/MetaballGridPhaseFieldFamily.ts`
- Critical behavioral delta for merge/review:
  - phase-field shape tuning was not missing from the render path,
  - it was being hidden by stale mask-texture reuse,
  - changing cell primitive shape during conquest should now refresh immediately.
