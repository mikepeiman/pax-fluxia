...
## ⭐ RULE 0.5 — ONE MASTER TASK LIST. CAPTURE TASKS THE MOMENT THEY'RE KNOWN.

There is exactly ONE master task list for the project: **`.agent/docs/MASTER_TASK_LIST.md`** (docs root). **Its purpose is to PREVENT FORGETTING AND LOSS OF TASKS.** The workflow is therefore **capture-first**: log a task the moment it becomes known — a user request, an idea, a bug you notice, a follow-up you realize, a "we should later…" — **BEFORE you implement it, not after.** Even a task you won't touch for weeks gets written down the instant it surfaces, even in the middle of other work. This is a forward-looking backlog to guarantee nothing is lost, not a retrospective changelog. Organize **by date** (newest first, under `## YYYY-MM-DD`, in an `### Open` list) plus any relevant context (area tag, mode, severity, owner, linked spec/plan/commit). Do NOT scatter task lists across ad-hoc docs. Same-day working detail may also live in the daily session queue (`.agent/docs/sessions/YYYY-MM-DD/`), but the durable, cross-day task-of-record is the master list. Marking items done (`[x]` + commit hash) is secondary bookkeeping — never delete history; the capture is the point.
