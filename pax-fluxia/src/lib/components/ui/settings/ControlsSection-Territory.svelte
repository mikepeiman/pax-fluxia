...
<div class="territory-section-shell territory-section-shell--renderer">
  <div class="territory-section-head">
    <h4 class="sub-heading territory-section-title">Territory Styles</h4>
    <div
      class="territory-scope-toggle"
      role="group"
...
{#if showRendererModule("metaball-grid") && isMetaballGridStyle() && !isMetaballGridPhaseFieldStyle()}
  <div class="engine-control-group territory-module-card">
    ...
    <div
      ...
      {:else if isMetaballGridPhaseFieldStyle()}
        Two-layer family: ownership geometry stays truth while the separate
        Phase Field variant reveals the NEXT geometry through a conquest-local
        cell mask instead of metaball blending.
      {:else}
    ...
    <div
      ...
      <strong>Topology lives in Territory Topology</strong> — use that panel
      for corridor, disconnect, star-margin, and frontier-resolution work.
      This styles panel is only for how the chosen territory mode looks and
      transitions.
    </div>
    <TerritorySurfaceStyleTuning
      {panel}
