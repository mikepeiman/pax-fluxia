...
{#if isPhaseEdgesMode()}
    <div class="mode-lock-note">
        Phase Edges starts from its own boundary defaults, but those values are now editable. Frontier technique comparison stays in this panel; visible fill, border, smoothing, trim, and border-geometry controls now live in <strong>Territory Styles</strong>.
    </div>
{/if}
...
<div class="var-row">
    ...
    <input
        ...
        max="4"
        step="1"
        disabled={!isPhaseEdgesMode() || !isControlFrontierTechnique() || currentFrontierBorderGeometryMode() !== 'shared_edge'}
