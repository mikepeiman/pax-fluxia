    ...
    const battles = $derived.by(() => {
        ...
        for (const log of sortedLogs) {
            ...
            if (existing && log.tick - existing.endTick <= 10 && existing.result === 'ONGOING') {
                ...
                // Start new battle
                // FIX: Include attacker ID to ensure uniqueness when multiple attackers hit same target at same tick
                const battle: Battle = {
                    id: `battle-${log.tick}-${log.attacker.ownerId}-${log.defender.id}`,
