<script lang="ts">
  ...
  function sanitizePlayerConfigs(
    input: PlayerConfig[],
    anchorHue: number,
  ): PlayerConfig[] {
    const normalized = makeDefaultPlayerConfigs(PLAYER_PALETTE_SIZE, anchorHue);
    const source = Array.isArray(input) ? input : [];
    const storedNudges = normalizePlayerPaletteNudges(
      storedPaletteSettings.nudges,
    );
  ...
  let selectedPaletteIndex = $state(
    Math.max(
      0,
      Math.min(loadSetting("selectedPaletteIndex", 0), PLAYER_PALETTE_SIZE - 1),
    ),
  );
  ...
  function selectPaletteIndex(index: number): void {
    selectedPaletteIndex = Math.max(
      0,
      Math.min(index, Math.max(0, playerCount - 1)),
    );
  }
  ...
  function getPlayerColorHex(index: number): string {
    const hue =
      (index < playerCount
        ? activePlayerPaletteHues[index]
        : fullPlayerPaletteHues[index]) ?? hueOffset;
    return hslToHex(hue);
  }
  ...
  function applyConfig() {
    ...
    const selectedMap = MAP_DEFS.find((m) => m.id === mapType) ?? MAP_DEFS[0];
  ...
  <div
    ...
    <div class="menu-container" transition:fly={{ y: 20, duration: 400 }}>
      ...
      <div class="menu-topbar">
        <div class="bg-picker">
          <button class="bg-picker-toggle" onclick={() => (bgOpen = !bgOpen)}>
            BG
          </button>
          {#if bgOpen}
            <div
        ...
        <div class="menu-topbar__right">
          <div class="audio-compact audio-compact--topbar">
            <button
              ...
              onclick={() => (audioManager.muted = !audioManager.muted)}
            ...
            {#if mapMode === "random"}
              ...
              <div class="map-col-content">
                <div class="config-row-3">
                  ...
                  <div class="config-item">
                    <label>Links <span>[{minLinks}-{maxLinks}]</span></label>
                    <div class="slider-container" style="padding: 0 4px;">
                      <RangeDual
                        bind:min={minLinks}
                        bind:max={maxLinks}
                        minLimit={1}
                ...
                <div class="config-row-3" style="grid-template-columns: 1fr;">
                  <div class="config-item">
                    <label
                      title="Loose: natural cluster. Tight: stars scaled to fill the padded map area."
                      >Board fill</label>
                    <div class="slider-container">
                      <input
                        type="range"
                ...
                <div class="config-row-3">
                  <div class="config-item">
                    <label title="Matches Map & Grid -> Lane clearance"
                      >Lane path</label>
                    <div
                  ...
                  <div class="config-item">
                    <label title="Territory / ownership boundary margin"
                      >MSR (px)</label>
                    <div class="slider-container">
                      <input
                        type="range"
                  ...
                  <div class="config-item">
                    <label
                      title="Lane chords & centerlines vs other stars (mapgen)"
                      >Lane margin (px)</label>
                    <div class="slider-container">
                      <input
                        type="range"
                        min="0"
                  ...
                  <div class="config-item">
                    <label
                      title="0 = prune tight chords; 1 = keep edges for curves"
                      >Curve vs prune</label>
                    <div class="slider-container">
                      <input
                        type="range"
                        min="0"
                        ...
                        oninput={() => persistMenuLaneKnobs()} />
                      <span class="value"
                        >{menuCurveVsPruneBias.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
                <div class="config-row-3">
                  <div class="config-item">
                    ...
                    <div class="slider-container">
                      <input
                        ...
                        max="100"
                        step="5"
                        bind:value={specialStarPercentage} />
                  ...
                  <div class="config-item">
                    ...
                    <div class="slider-container">
                      <input
                        ...
                        max="50"
                        step="1"
                        bind:value={neutralStarCount} />
                      <span class="value">{neutralStarCount}</span>
                    </div>
                  </div>
                  <div
                    class="config-item"
                    style={neutralStarCount === 0
                    ...
                    <div class="slider-container">
                      <input
                        ...
                        max="100"
                        step="5"
                        bind:value={neutralShipsPerStar} />
              ...
              <div class="classic-map-grid">
                {#each getClassicMaps() as m}
                  ...
                  <div
                    ...
                    class:selected={selectedClassicMap === m.metadata.name}
                    onclick={() => {
                      selectedClassicMap = m.metadata.name;
                    }}>
                    <svg
                      ...
                      {#each m.connections as conn}
                        {@const src = starMap[conn.sourceId]}
                        {@const tgt = starMap[conn.targetId]}
                        {#if src && tgt}
                          <line
                            x1={src.x}
                            y1={src.y}
                            x2={tgt.x}
                            y2={tgt.y}
                            stroke={selectedClassicMap === m.metadata.name
                              ? "#4488ff55"
                              : "#334466"}
                      ...
                      {#each m.stars as star}
                        <circle
                          ...
                          opacity={selectedClassicMap === m.metadata.name
                            ? 1
                            : 0.7} />
                      {/each}
                    </svg>
                    <span class="classic-card-label">{m.metadata.name}</span>
                    <span class="classic-card-info"
                      >{m.stars.length} stars</span>
                  </div>
                {/each}
                {#if getClassicMaps().length === 0}
                  <div class="no-maps-msg">No classic maps loaded yet</div>
                {/if}
              </div>
            {:else}
              <div class="classic-map-grid">
                {#each getCustomMaps() as m}
                  ...
                  <div
                    class="classic-map-card"
                    class:selected={selectedCustomMap === m.metadata.name}
                    onclick={() => {
                      selectedCustomMap = m.metadata.name;
                    }}>
                    <svg
                      class="classic-map-thumb"
                      viewBox="{minX} {minY} {vw} {vh}"
                      ...
                      {#each m.connections as conn}
                        ...
                        {#if src && tgt}
                          <line
                            ...
                            x2={tgt.x}
                            y2={tgt.y}
                            stroke={selectedCustomMap === m.metadata.name
                              ? "#4488ff55"
                              : "#334466"}
                            stroke-width={Math.max(1, vw * 0.006)} />
                        {/if}
                      ...
                      {#each m.stars as star}
                        <circle
                          ...
                          fill={star.ownerId === "neutral"
                            ? "#666"
                            : `hsl(${(m.stars.indexOf(star) * 60) % 360}, 70%, 60%)`}
                          opacity={selectedCustomMap === m.metadata.name
                            ? 1
                            : 0.7} />
                      {/each}
                    </svg>
                ...
                {#if getCustomMaps().length === 0}
                  <div class="no-maps-msg">No custom maps saved yet</div>
                {/if}
              </div>
            {/if}
          </div>
          ...
          <div class="config-row-3">
            ...
            <div class="config-item">
              ...
              <div class="slider-container">
                ...
                <span class="value">{(tickDuration / 1000).toFixed(1)}s</span>
              </div>
            </div>
          </div>
          ...
          <div class="player-colors-card player-colors-card--panel">
            <div class="player-colors-card__header">
              <div class="player-colors-card__heading">
                <span class="player-colors-card__title">Player Colors</span>
              </div>
              <div class="player-colors-card__actions">
                <button
                  ...
                  onclick={() =>
                    (showPaletteNudgePopover = !showPaletteNudgePopover)}
            ...
            <div class="player-colors-card__row">
              <label class="player-colors-card__label">Anchor Hue</label>
              <input
            ...
            <div class="player-colors-card__selection">
              <span class="player-colors-card__selection-label">Selected</span>
              <span class="player-colors-card__selection-value"
                >P{selectedPaletteIndex + 1} | {Math.round(selectedPaletteHue)} deg</span>
            </div>
              ...
              {#each activePlayerPaletteHues as hue, index}
                <button
                  ...
                  <span
                    ...
                    style="background: {getPlayerColorHex(index)}"></span>
              ...
              <div
                ...
                <div class="player-colors-card__focus-header">
                  ...
                  <span class="player-colors-card__focus-value"
                    >{Math.round(selectedPaletteHue)} deg</span>
                </div>
                <div
                  class="player-colors-card__row player-colors-card__row--nudge">
                  ...
                  <span class="value"
                    >{(playerConfigs[selectedPaletteIndex]?.hueNudge ?? 0) > 0
                      ? "+"
                      : ""}{playerConfigs[selectedPaletteIndex]?.hueNudge ?? 0} deg</span>
                </div>
                <div class="player-colors-card__row">
                ...
                <button
                  ...
                  disabled={(playerConfigs[selectedPaletteIndex]?.hueNudge ??
                    0) === 0}>
                  ...
                  <div
                    ...
                    {#if showAIDetails}
                      <select
                        ...
                        {#each AI_STRATEGIES as s}<option value={s.id}
                            >{s.label}</option
                          >{/each}
                      </select>
                    {/if}
        ...
        <section class="col-right panel compact-right">
          ...
          <div class="mp-section-compact">
            ...
            {#if multiplayerStore.isConnected}
              <div class="room-info-bar">
                <div class="room-id-block">
                  ...
                  <code class="room-code">{multiplayerStore.roomId}</code>
                  <button
                    class="copy-btn"
                    onclick={copyRoomId}
              ...
              <div class="lobby-actions">
                ...
                {:else}
                  <button
                    ...
                    onclick={() => multiplayerStore.voteToStart()}
                    ><span class="btn-glow"></span>VOTE TO START</button>
                  {#if multiplayerStore.startVoteInfo}
                    <span class="vote-progress">
                      {multiplayerStore.startVoteInfo.votes}/{multiplayerStore
                ...
                {#if multiplayerStore.isHost}
                  <button
                    ...
                    onclick={() => multiplayerStore.disposeRoom()}
                    >Dispose Room</button>
                {/if}
              </div>
              ...
              <div class="lobby-chat-section">
                <button
                  ...
                  {#if multiplayerStore.chatMessages.length > 0}<span
                      class="chat-count"
                      >{multiplayerStore.chatMessages.length}</span
                    >{/if}
                </button>
                {#if chatOpen}
                  <div
                    ...
                    {#if multiplayerStore.chatMessages.length === 0}
                      <p class="waiting-text">No messages yet</p>
                    {:else}
            ...
            {:else}
              <div class="mp-actions-compact">
                <button
                  ...
                  title={mapMode === "custom"
                    ...
                    : "Create multiplayer room"}>CREATE ROOM</button>
                <div class="join-inline">
                  ...
                  <button
                    ...
                    disabled={!joinRoomId.trim()}>JOIN</button>
                </div>
              </div>
              {#if mapMode === "custom"}
                ...
                {:else}
                  <div class="room-list">
                    {#each multiplayerStore.availableRooms as room}
                      ...
                      <div
                        class="room-card"
                        onclick={() => (confirmJoinTarget = room)}>
                        <div class="room-card-top">
                          ...
                          <div class="room-card-badges">
                            ...
                            {/if}
                            <span
                              class="room-phase badge {room.metadata?.phase ||
                                'lobby'}"
                              >{room.metadata?.phase || "lobby"}</span>
                          </div>
                        </div>
                        <div class="room-card-mid">
                          <span class="room-map"
                            >{room.metadata?.mapType || "?"}</span>
                          <span class="room-detail"
                            >Stars {room.metadata?.starsPerPlayer ||
                              "?"}/p</span>
                          <span class="room-detail"
                            >Ships {room.metadata?.shipsPerStar ||
                              "?"}/star</span>
                          {#if room.metadata?.phase === "playing" && room.metadata?.tick}
                            <span class="room-detail tick-badge"
                              >T{room.metadata.tick}</span>
                          {/if}
                        </div>
              ...
              {#if multiplayerStore.lobbyStatus}<div class="lobby-status-msg">
                  Status: {multiplayerStore.lobbyStatus}
                </div>{/if}
              {#if multiplayerStore.connectionError}<div class="error-msg">
...
{#if confirmJoinTarget}
  ...
  <div
    ...
    <div class="confirm-dialog" onclick={(e) => e.stopPropagation()}>
      ...
      <p>
        {confirmJoinTarget.clients}/{confirmJoinTarget.maxClients} players - {confirmJoinTarget
          .metadata?.mapType || "standard"}
      </p>
...
<style>
  ...
  .menu-columns {
    display: grid;
    grid-template-columns: minmax(0, 1.5fr) minmax(340px, 0.9fr) minmax(
        340px,
        0.95fr
      );
    gap: 16px;
