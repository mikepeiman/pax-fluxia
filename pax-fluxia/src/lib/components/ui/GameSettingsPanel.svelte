<script lang="ts">
  ...
  function applyConfigPatch(configPatch: Record<string, unknown>) {
    ...
    const affectsLanePaths =
      affectsLaneTopology || "MAPGEN_LANE_MODE" in configPatch;
      ...
      try {
        ...
        for (const [k, v] of Object.entries(incoming)) {
          ...
          if (
            ...
            acceptedPatch[k] = v;
            applied++;
          } else if (typeof existing === "boolean" && typeof v === "boolean") {
            acceptedPatch[k] = v;
            applied++;
          } else if (typeof existing === "string" && typeof v === "string") {
            acceptedPatch[k] = v;
            applied++;
          } else {
            typeErrors++;
  ...
  function recalcAnimLocksOnTickChange(newTickMs: number) {
    ...
    for (const [key, mode] of Object.entries(animLockModes)) {
      if (mode === "pinned" || mode === "ratio") {
        ...
        if (ratio != null) {
          const def = ANIM_SLIDERS.find((s) => s.key === key);
          let newVal = isTickRelativeSlider(def) ? ratio : ratio * newTickMs;
  ...
  function getThemeGroupSummary(group: ThemeFamilyGroup<GameTheme>): string {
    ...
    return [
      ...
      counts["legacy-fallback"]
        ...
        : "",
      counts["needs-editing"] ? `${counts["needs-editing"]} needs edit` : "",
  ...
  let { forceOpenSection = null, forceOpenSectionNonce = 0 }: Props = $props();
  ...
  function persistSectionOrder() {
    if (typeof window !== "undefined") {
      localStorage.setItem(ACTIVE_SECTION_KEY, JSON.stringify(sectionOrder));
    }
  }
  ...
  function setSectionSubsections(
    sectionId: SectionId,
    chips: SubsectionChip[],
  ) {
  ...
  function applySubsectionFilter(
    ...
    for (const child of Array.from(node.children) as HTMLElement[]) {
      ...
      const hidden =
        active !== "all" && subsectionId != null && subsectionId !== active;
  ...
  function scanSectionBody(
    ...
    for (const child of children) {
      ...
      if (subsectionHeading) {
        ...
        const id = subsectionIdFromLabel(label) || `sub-${chips.length + 1}`;
  ...
  <div class="full-theme-bar">
    ...
    {#if themeStore.allThemes.length > 0}
      <button
        ...
        <span class="ml-auto text-[10px] text-slate-500 font-normal"
          >{themeFamilyGroups.length} groups / {themeStore.allThemes
            .length}</span>
      </button>
      {#if showThemeChips}
          ...
          {#each themeFamilyGroups as group}
            <section class="theme-family-section">
              <div class="theme-family-header">
                ...
                <p class="theme-family-summary">
                  {getThemeGroupSummary(group)}
                </p>
              </div>
              <div class="full-chips-row">
                {#each group.themes as t}
                  ...
                  <button
                    ...
                    <span
                      class={`theme-chip-status ${getThemeStatusClass(routing.status)}`}>
            ...
            <div class="full-load-map-list">
              {#each getLoadableMaps() as map}
                <button
                  ...
                  <span class="full-load-map-item__name"
                    >{map.metadata.name}</span>
                  <span class="full-load-map-item__meta">
                    {Boolean((map as any).builtIn) ? "Classic" : "Custom"} · {map
                      .stars.length} stars · {map.connections.length} links
                  </span>
                </button>
              {/each}
            </div>
    ...
    <div class="section-panel" style="--accent: {sec.color}">
        ...
        {#if (sectionSubsections[sec.id]?.length ?? 0) > 0}
          <div class="section-subnav">
            ...
            {#each sectionSubsections[sec.id] ?? [] as subsection}
              <button
                class="subsection-chip"
                class:active={(activeSubsections[sec.id] ?? "all") ===
                  subsection.id}
                type="button"
      ...
      <div
        ...
        {:else if sec.id === "players"}
          <ControlsSectionPlayers syncFromConfig={syncAllFromConfig} />
...
<style>
  ...
  .tier-pill.active {
    ...
    box-shadow: 0 0 8px color-mix(in srgb, var(--tier-color) 20%, transparent);
  }
  .tier-icon {
    font-size: 11px;
  }
