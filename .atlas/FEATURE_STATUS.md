...
| B-42 | Territory border/fill misalignment — two root causes: (1) PIXI v8 missing `beginPath()` between fill/stroke/cut ops in PVV3 shell loop and border draw functions. (2) Legacy path applied Chaikin+Bézier smoothing to borders but not fills, creating geometric divergence. Fix: added `beginPath()` at 8 sites, eliminated smoothing divergence by passing `smoothPasses=0`. | ✅ Fixed | 2026-03-14 |
